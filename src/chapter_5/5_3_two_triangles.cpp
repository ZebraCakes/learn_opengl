...
int main(/*int arg_count, char** args*/)
{
    ...
    if (window)
    {
        ...
        if (gladLoadGLLoader((GLADloadproc)glfwGetProcAddress))
        {
            ...
            if (build_success)
            {
                const char* frag_shader_1_source =
                    "#version 330 core\n"
                    "out vec4 FragColor;\n"
                    "void main()\n{"
                ...
                glShaderSource(frag_shader, 1, &frag_shader_1_source, nullptr);
                glCompileShader(frag_shader);
                glGetShaderiv(frag_shader, GL_COMPILE_STATUS, &build_success);
                if (build_success)
                {
                    u32 shader_program_1;
                    shader_program_1 = glCreateProgram();
                    glAttachShader(shader_program_1, vertex_shader);
                    glAttachShader(shader_program_1, frag_shader);
                    glLinkProgram(shader_program_1);

                    glGetProgramiv(shader_program_1, GL_LINK_STATUS, &build_success);
                    if (build_success)
                    {
                        glDeleteShader(frag_shader);

                        const char* frag_shader_2_source =
                            "#version 330 core\n"
                            "out vec4 FragColor;\n"
                            "void main()\n{"
                            "    FragColor = vec4(1.0f, 1.0f, 0.2f, 1.0f);\n}";
                        frag_shader = glCreateShader(GL_FRAGMENT_SHADER);
                        glShaderSource(frag_shader, 1, &frag_shader_2_source, nullptr);
                        glCompileShader(frag_shader);
                        glGetShaderiv(frag_shader, GL_COMPILE_STATUS, &build_success);
                        if (build_success)
                        {
                            u32 shader_program_2;
                            shader_program_2 = glCreateProgram();
                            glAttachShader(shader_program_2, vertex_shader);
                            glAttachShader(shader_program_2, frag_shader);
                            glLinkProgram(shader_program_2);

                            glGetProgramiv(shader_program_2, GL_LINK_STATUS, &build_success);
                            if (build_success)
                            {
                                glDeleteShader(frag_shader);
                            ...
                            {
                                glGetProgramInfoLog(shader_program_2, sizeof(log), nullptr, log);
                    ...
                    {
                        glGetProgramInfoLog(shader_program_1, sizeof(log), nullptr, log);
                        printf("ERROR::SHADER::LINK_FAILED:\n\t%s", log);
                    }
                }
                else
